...
const initializeGoogleSheets = async () => {
  if (!GOOGLE_SHEET_ID || !GOOGLE_SERVICE_ACCOUNT_EMAIL || !GOOGLE_PRIVATE_KEY) {
    console.log('Google Sheets not configured. Skipping spreadsheet integration.');
    return null;
  }
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p style="color: #6b7280; font-size: 14px;">
            This booking was submitted on ${new Date(bookingData.timestamp).toLocaleString()}
          </p>
        </div>
      `
    };
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!bookingData.name || !bookingData.email || !bookingData.company || !bookingData.preferredDate || !bookingData.preferredTime) {
      return res.status(400).json({
        success: false,
        message: 'Missing required fields: name, email, company, preferredDate, preferredTime'
      });
    }
